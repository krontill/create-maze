...
name: commit
description: >
  Stage changed files and create a well-formed commit following Conventional
  Commits. Use when the user wants to commit, stage files, or create a git
  commit message. Triggers on "commit", "stage", "git commit", "create a commit".
---
...
1. Run `git status` to see what is changed and untracked (use `powershell` tool).
2. Run `git diff` (staged and unstaged) to understand what changed and why.
3. Run `git log --no-pager --oneline -10` to match this repo's existing commit style.
4. Ask the user which files to stage if it is not obvious from context — never silently stage everything.
...
6. Draft a commit message:
   ...
   - End with `Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>` trailer
...
All git commands run via the `powershell` tool. On Windows, use single quotes carefully — prefer double quotes for commit messages with the `-m` flag, or use a temporary file if the message contains special characters.
