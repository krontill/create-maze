---
name: changelog
description: Generate a human-readable changelog from git history. Use when the user wants to generate or write a changelog, release notes, or summarize recent commits.
argument-hint: [since]
...
1. Determine the range:
   ...
   - Otherwise find the latest tag with `git describe --tags --abbrev=0` and use that as the base
   - If no tags exist, use the last 50 commits
2. Run `git log <range> --pretty=format:"%h %s (%an)" --no-merges`
...
5. Ask: "Write to CHANGELOG.md?" — prepend to existing file if confirmed
